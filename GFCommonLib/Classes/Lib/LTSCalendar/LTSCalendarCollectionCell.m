...
- (void)setItem:(LTSCalendarDayItem *)item{
    ...
    dotView.color = [UIColor colorWithRed:255/255.0 green:228/255.0 blue:139/255.0 alpha:0.4];
...
- (void)configureConstraintsForSubviews{
    ...
    dotView.frame = CGRectMake(0, 0, sizeCircle, sizeCircle);
    dotView.center = CGPointMake(self.frame.size.width / 2., circleView.center.y);
    
    dotView.layer.cornerRadius = dotView.width / 2.;
...
- (void)setSelected:(BOOL)isSelected animated:(BOOL)animated{
    ...
    if(isSelected){
        ...
        if ([self isToday]){
            ...
            textLabel.textColor = [UIColor colorWithHexString:@"FFE48B"];
